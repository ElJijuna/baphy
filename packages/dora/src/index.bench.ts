...
import { calcDeploymentFrequency, calcLeadTime, calcChangeFailureRate, calcMttr, calcDora } from './index.js'
...
function makeChanges(count: number, leadHours = 4): ChangeEvent[] {
  ...
  return Array.from({ length: count }, (_, i) => ({
    ...
    deployedAt: new Date(base.getTime() - (count - i) * 24 * 60 * 60 * 1000 + leadHours * 60 * 60 * 1000),
  }))
}
...
function makeIncidents(count: number, mttrHours = 0.5): IncidentEvent[] {
  ...
  return Array.from({ length: count }, (_, i) => ({
    ...
    restoredAt: new Date(base.getTime() - (count - i) * 24 * 60 * 60 * 1000 + mttrHours * 60 * 60 * 1000),
  }))
}
